...
export const projects: Project[] = [
  {
    ...
    problem: "Corn leaf disease identification is still done manually, which is time-consuming and prone to subjective assessment. An automated mobile-based system is needed to detect diseases quickly and accurately.",
    dataset: [
      { label: "Images", value: "5,261" },
      { label: "Classes", value: "4 (Blight, Common Rust, Gray Leaf Spot, Healthy)" },
      { label: "Split", value: "70:20:10 (train:val:test)" },
      { label: "Annotation", value: "Bounding box via Roboflow" },
    ],
    model: { name: "YOLOv8n", task: "Object detection — 50 epochs, Adam optimizer, image size 640" },
    results: [
    ...
    implementation: ["TensorFlow Lite on-device inference", "Flutter mobile app", "Node.js REST API", "MySQL detection history", "Camera & gallery detection", "Bounding boxes + confidence scores", "Early treatment recommendations"],
    conclusion: "YOLOv8n outperformed YOLO11n and Faster R-CNN ResNet-50, achieving the highest precision, recall, and mAP50. The model was successfully deployed to a mobile device achieving near real-time detection (~1.6 FPS).",
...

  {
    ...
    problem: "Farmers need a practical web-based tool to identify corn leaf diseases from images and receive treatment guidance without relying on manual visual inspection.",
    dataset: [
      { label: "Images", value: "4,188" },
      { label: "Classes", value: "4 (Blight, Common Rust, Gray Leaf Spot, Healthy)" },
      { label: "Split", value: "80:10:10 (train:val:test)" },
    ],
    model: { name: "ConvNeXt-Small", task: "Multi-class image classification — fine-tuned on ImageNet weights" },
    results: [
      { value: "97.31%", label: "Test accuracy" },
    ],
    implementation: ["Flask web application", "Image upload interface", "Top-three predictions with confidence scores", "UNCERTAIN status for low confidence", "Treatment recommendations per disease class"],
    conclusion: "ConvNeXt-Small achieved 97.31% test accuracy across 4 classes, outperforming EfficientNet-B0 (96.28%) and ResNet-50 (91.24%). The system was presented at ICOSA 2026.",
...

  {
    ...
    problem: "Financial recording at the RT level is still done manually using notebooks, leading to slow reporting, data errors, and limited transparency for residents.",
    implementation: ["Flutter mobile app (admin & resident roles)", "Firebase authentication", "MySQL database", "RT fund income/expense management", "Resident savings tracking", "QR barcode scanning", "Activity/event management", "Financial transaction history"],
    conclusion: "The application successfully digitizes financial recording for RT 03/RW 09 Desa Klapagading, reducing manual errors and improving transparency. Registered as a copyrighted computer program with DJKI (Reg. No. EC002026071168).",
...

  {
    ...
    problem: "Depression among university students has a high prevalence but is often undetected. Conventional screening relies on psychological scales that require active participation. A data-driven approach can support early identification.",
    dataset: [
    ...
    model: { name: "Logistic Regression", task: "Binary classification — compared with Random Forest" },
    results: [
      { value: "81.0%", label: "Test accuracy" },
      { value: "0.755", label: "ROC-AUC (LR)" },
      { value: "0.621", label: "F1 CV (LR)" },
    ],
    implementation: ["Preprocessing pipeline with ColumnTransformer", "Feature engineering (year extraction, CGPA binning)", "5-fold stratified cross-validation", "Feature influence analysis (LR coefficients + RF importance)", "ROC-AUC probability analysis"],
    conclusion: "Logistic Regression achieved 81.0% test accuracy with better probability calibration (ROC-AUC 0.755) than Random Forest (0.735). Top predictive features included CGPA, study year, and sleep quality.",
